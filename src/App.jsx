...
const CUSTOMER_STORAGE_KEY = "gardslattur-bjarka-customers-v4";
const PLAN_STORAGE_KEY = "gardslattur-bjarka-plan-v4";
const LOCATION_STORAGE_KEY = "gardslattur-bjarka-locations-v2";
const DAY_TIMER_STORAGE_KEY = "gardslattur-bjarka-day-timer-v2";
const DAY_HISTORY_STORAGE_KEY = "gardslattur-bjarka-day-history-v1";
const EXPENSE_STORAGE_KEY = "gardslattur-bjarka-expenses-v3";
...
function parseIcelandicReceiptQr(raw) {
  ...
  for (let i = 0; i < datePatterns.length; i += 1) {
...
export default function App() {
  ...
  const [dayHistory, setDayHistory] = useState(() => {
    try {
      const saved = localStorage.getItem(DAY_HISTORY_STORAGE_KEY);
      return saved ? JSON.parse(saved) : {};
    } catch {
      return {};
    }
  ...
  useEffect(() => {
    try {
      localStorage.setItem(DAY_HISTORY_STORAGE_KEY, JSON.stringify(dayHistory));
    } catch {}
  }, [dayHistory]);
  ...
  const getUnloggedMinutesForDate = (dateKey) => {
    const worked = dayHistory[dateKey]?.workedMinutes || 0;
    const logged = logs
  ...
  const getLiveWorkedMinutes = (timerState, nowOverride = null) => {
    ...
    const endTime = timerState.running ? nowOverride || timerNow : timerState.dayEndedAt;
    if (!endTime) return 0;
  ...
  const monthCells = useMemo(
    () => buildCalendar(monthDateObj.getFullYear(), monthDateObj.getMonth()),
    [monthDateObj]
  );
    ...
    >
      <div style={{ maxWidth: 900, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 34, fontWeight: 900, letterSpacing: "-0.05em", lineHeight: 1 }}>
              Garðsláttur Bjarka
            </div>
            <div style={{ color: "#64748b", marginTop: 6 }}>Snirtilegt mobile app fyrir slætti</div>
          </div>
          <div style={{ fontSize: 18, fontWeight: 800, color: "#1d4ed8" }}>{new Date().getFullYear()}</div>
        </div>
          ...
          <div style={{ display: "grid", gap: 16 }}>
            <div style={cardStyle({ background: "linear-gradient(180deg, rgba(255,255,255,0.96), rgba(219,234,254,0.95))" })}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                <div>
                  <div style={{ fontSize: 28, fontWeight: 900 }}>Dagstími</div>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end", marginTop: 12 }}>
                {!editingDayTimer ? (
                  <button style={buttonStyle(false)} onClick={() => startEditDayTimer(todayDate)}>
                    Edita dagstimer
                  </button>
                ) : null}
              </div>
              ...
              >
                <div style={{ background: "#f8fafc", borderRadius: 22, padding: 14 }}>
                  ...
                  <div style={{ fontWeight: 900, fontSize: 24 }}>
                    {formatClockTime(dayHistory[todayDate]?.startTime || dayTimerState.dayStartedAt)}
                  </div>
                </div>
                ...
                <div style={{ background: "#f8fafc", borderRadius: 22, padding: 14 }}>
                  ...
                  <div style={{ fontWeight: 900, fontSize: 24 }}>
                    {formatClockTime(dayHistory[todayDate]?.endTime || (dayTimerState.running ? null : dayTimerState.dayEndedAt))}
                  </div>
                </div>
                ...
                <div style={{ background: "#f8fafc", borderRadius: 22, padding: 14 }}>
                  ...
                  <div style={{ fontWeight: 900, fontSize: 24 }}>
                    {minsToText(Math.floor((dayHistory[todayDate]?.breakMs || dayTimerState.breakMs || 0) / 60000))}
                  </div>
                </div>
              </div>
              ...
              {editingDayTimer && (
                <div style={cardStyle({ marginTop: 12 })}>
                  <div style={{ fontSize: 20, fontWeight: 900, marginBottom: 12 }}>Edita dagstimer</div>
            ...
            <div style={cardStyle()}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(7, 1fr)", gap: 6 }}>
                {monthCells.map((cell, i) => {
                  if (!cell) {
                    return <div key={i} style={{ minHeight: 104, borderRadius: 20, background: "rgba(226,232,240,0.45)" }} />;
                  }
                  ...
                  return (
                    ...
                    >
                      ...
                      {dayLogs.slice(0, 2).map((log) => (
                        <div
                          key={log.id}
                          style={{ fontSize: 12, color: "#334155", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}
                        >
                          {log.customer}
                        </div>
                      ))}
                ...
                <div style={{ padding: 14, display: "grid", gap: 10 }}>
                  ...
                  >
                    <div style={{ background: "#f8fafc", borderRadius: 18, padding: 12 }}>
                      <div style={{ color: "#64748b", fontSize: 13 }}>Dagstími</div>
                      <div style={{ fontWeight: 900, fontSize: 20 }}>
                        {minsToText(dayHistory[selectedDay]?.workedMinutes || 0)}
                      </div>
                    </div>
                    ...
                    <div style={{ background: "#f8fafc", borderRadius: 18, padding: 12 }}>
                      ...
                      <div style={{ fontWeight: 900, fontSize: 20 }}>
                        {minsToText(getUnloggedMinutesForDate(selectedDay))}
                      </div>
                    </div>
                    ...
                    <div style={{ background: "#f8fafc", borderRadius: 18, padding: 12 }}>
                      ...
                      <div style={{ fontWeight: 900, fontSize: 20 }}>
                        {formatClockTime(dayHistory[selectedDay]?.startTime)}
                      </div>
                    </div>
                    ...
                    <div style={{ background: "#f8fafc", borderRadius: 18, padding: 12 }}>
                      ...
                      <div style={{ fontWeight: 900, fontSize: 20 }}>
                        {formatClockTime(dayHistory[selectedDay]?.endTime)}
                      </div>
                    </div>
                  </div>
                      ...
                      {editingLogId === log.id ? (
                        <div style={{ display: "grid", gap: 10 }}>
                          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(140px,1fr))", gap: 10 }}>
                            <input style={inputStyle()} value={editForm.customer} onChange={(e) => setEditForm({ ...editForm, customer: e.target.value })} />
                            ...
                            <input style={inputStyle()} value={editForm.note} onChange={(e) => setEditForm({ ...editForm, note: e.target.value })} />
        ...
        {screen === "Í dag" && (
          <div style={{ display: "grid", gap: 16 }}>
            <div style={cardStyle({ background: "linear-gradient(180deg, rgba(255,255,255,0.96), rgba(219,234,254,0.95))" })}>
              ...
              {editingDayTimer && selectedDay === todayDate && (
                <div style={cardStyle({ marginTop: 12 })}>
                  <div style={{ fontSize: 20, fontWeight: 900, marginBottom: 12 }}>Edita dagstimer</div>
              ...
              >
                ...
                <div style={{ background: "#f8fafc", borderRadius: 22, padding: 14 }}>
                  <div style={{ color: "#475569", fontSize: 13 }}>Óskráður tími</div>
                  <div style={{ fontWeight: 900, fontSize: 24 }}>
                    {minsToText(getUnloggedMinutesForDate(todayDate))}
                  </div>
                </div>
              </div>

              <div style={cardStyle({ marginTop: 16 })}>
                <div style={{ fontSize: 22, fontWeight: 900, marginBottom: 10 }}>Plan dagsins</div>
                ...
                {todayPlanLines.length > 0 ? (
                  ...
                  <div style={{ color: "#64748b" }}>Ekkert plan skráð í dag.</div>
                )}
              </div>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(150px,1fr))", gap: 12, marginTop: 16 }}>
                <div style={{ background: "#dbeafe", borderRadius: 22, padding: 14 }}>
                  ...
                  <div style={{ fontWeight: 900, fontSize: 24 }}>
                    {kr(myDayLogs.reduce((s, l) => s + l.earned, 0))}
                  </div>
                </div>
                ...
                <div style={{ background: "#ede9fe", borderRadius: 22, padding: 14 }}>
                  ...
                  <div style={{ fontWeight: 900, fontSize: 24 }}>
                    {minsToText(myDayLogs.reduce((s, l) => s + l.minutes, 0))}
                  </div>
                </div>
              ...
              {myDayLogs.length === 0 && (
                <div style={cardStyle()}>
                  ...
                  <div style={{ color: "#64748b", marginTop: 6 }}>
                    Farðu í Skrá og bættu við slætti til að sjá timeline hér.
                  </div>
                </div>
              )}
                ...
                <div key={log.id} style={cardStyle()}>
                  {editingLogId === log.id ? (
                    ...
                    <>
                      ...
                      <div style={{ display: "flex", justifyContent: "space-between", gap: 8, flexWrap: "wrap", alignItems: "center", marginTop: 12 }}>
                        <label style={{ display: "flex", alignItems: "center", gap: 10, fontWeight: 700, color: "#334155" }}>
                          <input type="checkbox" checked={log.paid} onChange={() => togglePaid(log.id)} />
                          {log.paid ? "Greitt" : "Ógreitt"}
                        </label>
          ...
          <div style={{ display: "grid", gap: 16 }}>
            ...
            >
              <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
                ...
                <div style={{ padding: "8px 12px", borderRadius: 999, background: "rgba(29,78,216,0.08)", color: "#1d4ed8", fontWeight: 800 }}>
                  Quick add
                </div>
              </div>
              ...
              <div style={{ display: "grid", gap: 12 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(2, minmax(0,1fr))", gap: 12 }}>
                  ...
                  <div>
                    ...
                    <div style={{ ...inputStyle(), display: "flex", alignItems: "center", fontWeight: 900, fontSize: 24, background: "#f8fafc" }}>
                            ...
                            <div style={{ display: "flex", justifyContent: "space-between", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                              <div>
                                ...
                                <div style={{ color: "#64748b", marginTop: 4 }}>
                                  {client.pricing === "hourly"
                                    ? `Tímakaup ${kr(client.price)}/klst`
                                    : `Fast verð ${kr(client.price)}`}
                                </div>
                              </div>
          ...
          <div style={{ display: "grid", gap: 16 }}>
            <div style={cardStyle()}>
              ...
              >
                <div>
                  ...
                  <div style={{ color: "#64748b", marginTop: 4 }}>Ýttu á mánuð til að opna hann</div>
                </div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(220px,1fr))", gap: 12 }}>
              <div style={cardStyle({ background: "linear-gradient(135deg,#fef9c3 0%, #fde68a 100%)" })}>
                <div style={{ color: "#475569", fontSize: 13 }}>🏆 Besti dagur</div>
              ...
              <div style={cardStyle({ background: "linear-gradient(135deg,#e0e7ff 0%, #c7d2fe 100%)" })}>
                <div style={{ color: "#475569", fontSize: 13 }}>⏱️ Lengsti dagur</div>
                <div style={{ fontSize: 22, fontWeight: 900, marginTop: 6 }}>
            ...
            <div style={cardStyle()}>
              ...
              <div style={{ marginTop: 14, display: "grid", gap: 12 }}>
                ...
                <div style={{ height: 520, borderRadius: 24, overflow: "hidden", border: "1px solid #dbe2ea" }}>
                  <MapContainer center={AKUREYRI_CENTER} zoom={13} scrollWheelZoom style={{ height: "100%", width: "100%" }}>
        ...
        {screen === "Skrifa kostnað" && (
          <div style={{ display: "grid", gap: 16 }}>
            <button style={{ ...buttonStyle(false), width: "fit-content" }} onClick={() => setScreen("Kostnaður")}>
              ← Til baka
            </button>
                ...
                {expenseForm.category === "fuel" && (
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) => setExpenseForm((prev) => ({ ...prev, fuelType: e.target.value }))}
                    >
              ...
              {scanPreview ? (
                <div style={{ display: "grid", gap: 10 }}>
                  ...
                  <div style={{ color: "#64748b" }}>
                    Þú getur lagað upplýsingarnar handvirkt hér fyrir neðan áður en þú vistar.
                  </div>
                </div>
              ) : (
            ...
            <div style={cardStyle()}>
              ...
              <div style={{ display: "grid", gap: 12 }}>
                ...
                <select
                  style={inputStyle()}
                  value={expenseForm.fuelType}
                  onChange={(e) => setExpenseForm((prev) => ({ ...prev, category: "fuel", fuelType: e.target.value }))}
                >
                  <option value="diesel">Dísel</option>
                  <option value="95">95</option>
                  <option value="98">98</option>
                ...
                <input
                  style={inputStyle()}
                  placeholder="Raw QR text / athugasemd"
                  value={expenseForm.note}
                  onChange={(e) => setExpenseForm((prev) => ({ ...prev, note: e.target.value }))}
                />
              </div>
                      ...
                      {hasPlan ? (
                        <div style={{ marginTop: 6, display: "grid", gap: 2 }}>
                          {previewShort.map((line, idx) => (
                            <div key={idx} style={{ fontSize: 11, lineHeight: 1.15, color: "#1e3a8a", fontWeight: 800 }}>
                              {line}
                            </div>
                          ))}
